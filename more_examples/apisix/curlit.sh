#!/bin/bash
#
# curlit.sh - Test APISIX server with and without client certificates
#
...
BASE_URL_TLS="https://localhost:9443"
BASE_URL_MTLS="https://localhost:9444"
...
echo -e "${BLUE}==================================================================${COFF}"
echo -e "${BLUE}  Apache APISIX TLS/mTLS Demo - Testing Endpoints${COFF}"
echo -e "${BLUE}==================================================================${COFF}"
echo ""
...
echo -e "${GREEN}Test 1: Accessing standard TLS endpoint (port 9443)${COFF}"
...
echo -e "${YELLOW}Test 2: Accessing mTLS endpoint (port 9444) WITHOUT client certificate${COFF}"
...
if curl -s --cacert "${CA_BUNDLE}" "${BASE_URL_MTLS}/" -o /tmp/apisix-test-nocert.html 2>&1 | grep -q "SSL"; then
...
if [ -f "${CLIENT_CERT_DIR}/cert.pem" ] && [ -f "${CLIENT_CERT_DIR}/key.pem" ]; then
    echo -e "${GREEN}Test 3: Accessing mTLS endpoint (port 9444) WITH client certificate${COFF}"
    ...
    curl --cacert "${CA_BUNDLE}" \
         ...
         "${BASE_URL_MTLS}/" 2>/dev/null > /tmp/apisix-test-withcert.html
    
    if grep -q "Success" /tmp/apisix-test-withcert.html; then
        echo -e "${GREEN}✓ Successfully authenticated with client certificate!${COFF}"
        echo ""
        echo "Response excerpt:"
        grep -o "<h1>.*</h1>" /tmp/apisix-test-withcert.html || true
    else
...
echo ""
echo "Summary:"
echo "  ✓ Port 9443: Standard TLS - No client cert required"
echo "  ✓ Port 9444: Strict mTLS - Client cert REQUIRED"
echo ""
echo "Admin API: http://localhost:9180/apisix/admin"
echo "Docker logs: docker logs demo-apisix"
echo "Stop server: docker compose down"
echo ""

# Cleanup
rm -f /tmp/apisix-test-nocert.html /tmp/apisix-test-withcert.html
