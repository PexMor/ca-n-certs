#!/bin/bash
#
# start.sh - Easy startup script for Caddy TLS/mTLS demo
#
...
echo -e "${BLUE}==================================================================${COFF}"
echo -e "${BLUE}  Caddy TLS/mTLS Demo - Startup Script${COFF}"
echo -e "${BLUE}==================================================================${COFF}"
echo ""
...
if [ ! -f "${BD}/ca-bundle-myca.pem" ]; then
    ...
    echo "This will create ca-bundle-myca.pem with your Root CA + ICA"
    exit 1
fi
echo -e "${GREEN}✓ CA bundle exists (ca-bundle-myca.pem)${COFF}"
...
# Check client certificates (optional but recommended)
if [ ! -f "${BD}/smime-openssl/john_extended/cert.pem" ]; then
    echo -e "${YELLOW}⚠ Client certificate not found (optional)${COFF}"
    echo "  The /client endpoint will not work until you create client certificates."
    echo "  Run: cd ../.. && ./steps.sh step_email_openssl john_extended john@example.com"
    echo ""
else
...
echo ""
echo -e "${YELLOW}Starting Caddy server...${COFF}"

# Start Docker Compose
docker compose up -d

echo ""
echo -e "${GREEN}✓ Caddy server started successfully!${COFF}"
echo ""
echo -e "${BLUE}==================================================================${COFF}"
echo -e "${BLUE}  Server Information${COFF}"
echo -e "${BLUE}==================================================================${COFF}"
echo ""
echo "Container: demo-caddy"
echo "URL: https://localhost:8443"
echo ""
echo "Available endpoints:"
echo "  - https://localhost:8443/        → Standard TLS (no client cert)"
echo "  - https://localhost:8443/client  → mTLS required (client cert)"
...
echo -e "  ${GREEN}curl --cacert ${BD}/ca-bundle-myca.pem https://localhost:8443/${COFF}"
echo ""
echo "View logs:"
echo -e "  ${GREEN}docker logs demo-caddy${COFF}"
echo ""
echo "Follow logs:"
echo -e "  ${GREEN}docker logs -f demo-caddy${COFF}"
echo ""
echo "Stop server:"
...
if curl -s --cacert "${BD}/ca-bundle-myca.pem" https://localhost:8443/health > /dev/null 2>&1; then
    ...
    echo "Check logs with: docker logs demo-caddy"
fi
echo ""
