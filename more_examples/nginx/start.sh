#!/bin/bash
#
# start.sh - Easy startup script for Nginx TLS/mTLS demo
#
...
echo -e "${BLUE}==================================================================${COFF}"
echo -e "${BLUE}  Nginx TLS/mTLS Demo - Startup Script${COFF}"
echo -e "${BLUE}==================================================================${COFF}"
echo ""
...
echo ""
echo -e "${YELLOW}Starting Nginx server...${COFF}"

# Start Docker Compose
docker compose up -d

echo ""
echo -e "${GREEN}✓ Nginx server started successfully!${COFF}"
echo ""
echo -e "${BLUE}==================================================================${COFF}"
echo -e "${BLUE}  Server Information${COFF}"
echo -e "${BLUE}==================================================================${COFF}"
echo ""
echo "Container: demo-nginx"
echo ""
...
echo -e "  ${GREEN}curl --cacert ${BD}/ca-bundle-myca.pem --cert ${BD}/tls-clients/john_tls_client/cert.pem --key ${BD}/tls-clients/john_tls_client/key.pem https://localhost:8444/${COFF}"
echo ""
echo "View logs:"
echo -e "  ${GREEN}docker logs demo-nginx${COFF}"
echo ""
echo "Follow logs:"
echo -e "  ${GREEN}docker logs -f demo-nginx${COFF}"
echo ""
echo "Stop server:"
...
if curl -s --cacert "${BD}/ca-bundle-myca.pem" https://localhost:8443/health > /dev/null 2>&1; then
    ...
    echo "Check logs with: docker logs demo-nginx"
fi
echo ""
